...
@cython.boundscheck(False)
cdef int32 _get_increasing_leaf_node_pairs_array(float64 *rule_lower_corners,
                           float64 *rule_upper_corners,
                           int32 *leaf_ids,
                           int32 *feat_types,
                           Py_ssize_t n_rules,
    ...
    cdef Py_ssize_t j_
    n_pairs=0
    for i_ in range(n_rules):
        i=leaf_ids[i_]
        for j_ in range(n_rules):
            j=leaf_ids[j_]
            if i!=j:
                res=1
                k=0
...
def get_increasing_leaf_node_pairs_array(object rule_lower_corners, 
                                   object rule_upper_corners,
                                   object leaf_ids,
                                   object  feat_types,
                                   np.ndarray[int32, ndim=2] out):

    n_pairs_=_get_increasing_leaf_node_pairs_array(<float64*> (<np.ndarray> rule_lower_corners).data, 
                                    <float64*> (<np.ndarray> rule_upper_corners).data,
